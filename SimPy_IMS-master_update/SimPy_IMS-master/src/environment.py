...
class Procurement:
    ...
    def order_material(self, supplier, inventory, daily_events, lead_time_dict):
        ...
        while True:
            ...
            # Check inventory levels and decide order size
            if SSPOLICY:
                # When SSPOLICY is True, use the S-Policy for reordering
                if inventory.on_hand_inventory + inventory.in_transition_inventory <= SQPAIR['Reorder']:
                    order_size = SQPAIR['Order']
                else:
                    order_size = 0
            else:
                # When SSPOLICY is False, use REORDER_LEVEL and ORDER_QTY for reordering
                if inventory.on_hand_inventory + inventory.in_transition_inventory <= REORDER_LEVEL:
                    order_size = ORDER_QTY
                else:
                    order_size = 0

            # Place the order if order_size > 0
            if order_size > 0:
                daily_events.append(
                ...
                # 시간 조정 후 재고 상태 업데이트
                yield self.env.timeout(1)  # 시간을 적절히 조정하여 상태 반영
                ...
                # Record inventory (Total_Inventory)
                daily_events.append(
...
class Cost:
    ...
    def update_cost_log(inventoryList):
        ...
        # Log cumulative cost
        if LOG_CUMULATIVE_COST:
            LOG_CUMULATIVE_COST.append(LOG_CUMULATIVE_COST[-1] + LOG_COST[-1])
        else:
            LOG_CUMULATIVE_COST.append(LOG_COST[-1])
